...
function GetDoctorName(obUserName) {
    var requestdata = JSON.stringify({ UserName: obUserName });
    $.ajax({
        type: "POST",
        url:"../DoctorServices.asmx/GetFromStaff",
        data: requestdata,
        dataType: "json",
        contentType: "application/json; charset=utf-8",
        success: function(response) {
            var doctor = response.d;
          $("[id$=dataDoctorShortName]").val(doctor.ShortName);
...
function GetDoctor() {
    ...
    var userName=$("[id$=cboDoctorUserName]").val();
    GetDoctorName(userName);
